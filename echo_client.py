...
async def chat_client_proto(scope: Dict, conn: EchoQuicConnection):
    ...
    state = "USER_SELECTION"  # Initialize the state to USER_SELECTION
        ...
        try:
            while True:
                ...
                if dgram_resp.mtype == pdu.MSG_TYPE_AUTH_ACK:
                    ...
                    pass  # Presence acknowledgement received, do nothing
                elif dgram_resp.mtype == pdu.MSG_TYPE_CHAT_ACK:
                    pass  # Chat message acknowledgement received, do nothing
                elif dgram_resp.mtype == pdu.MSG_TYPE_CHAT_NACK:
                    pass  # Chat message not acknowledged (recipient not available), do nothing
                else:
    ...
    while True:
        if state == "USER_SELECTION":
            ...
            if recipient_username == "quit":
                ...
                )  # Send presence message to update offline status
                break
            state = "CHAT"  # Switch to CHAT state
            input_event.clear()
            ...
            if input_task in done:
                ...
                if chat_message == "back":
                    state = "USER_SELECTION"  # Switch back to USER_SELECTION state
                    print(
                        "\033c", end=""
                    )  # Clear the console before showing online users
                ...
                try:
                    ...
                    pass
                state = "USER_SELECTION"  # Switch back to USER_SELECTION state
                print("\033c", end="")  # Clear the console before showing online users
